...
from emissary.decision import FinalOutput, ModelCapabilities, ModelResult, ToolDefinition, Usage
from emissary.messages import TextBlock, UserMessage
from emissary.model import FallbackModelCaller, call_model
...
def test_call_model_gates_credentials_before_wire_dispatch(monkeypatch):
    ...
    with (
        patch("emissary.model.anthropic_wire.call_model") as wire,
...
def test_call_model_dispatches_by_wire_and_returns_normalized_result(monkeypatch):
    ...
    with patch("emissary.model.anthropic_wire.call_model", return_value=RESULT) as wire:
...
def test_fallback_caller_retries_only_availability_failures():
    ...
    with patch("emissary.model.call_model", side_effect=outcomes) as called:
        assert caller(system="s", messages=MESSAGES) is RESULT
    assert called.call_count == 2

    with (
        patch(
            "emissary.model.call_model", side_effect=ProviderError("malformed", retryable=False)
        ) as called,
...
def test_fallback_caller_does_not_repeat_the_same_provider():
    ...
    with (
        patch(
            "emissary.model.call_model", side_effect=ProviderError("down", retryable=True)
        ) as called,
...
def test_unsupported_tool_capability_fails_before_wire_dispatch(monkeypatch):
    ...
    with (
        patch("emissary.model.openai_wire.call_model") as wire,
